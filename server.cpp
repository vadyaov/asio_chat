...
private:
  ...
  void do_read_header() {
    asio::async_read(socket_,
        asio::buffer(&read_message_.header, sizeof(decltype(read_message_.header))),
  ...
  void do_write() {
    asio::async_write(socket_, asio::buffer(&write_msgs_.front(), sizeof(chat_message)),
        std::bind(&chat_session::handle_write, shared_from_this(), asio::placeholders::error, asio::placeholders::bytes_transferred));
  }
